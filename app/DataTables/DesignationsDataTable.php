...
class DesignationsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (Designation $designation) {
                ...
                return view('pages.apps.designations.columns._actions', $pageData);
            })
            ->editColumn('created_at', function (Designation $designation) {
                return $designation->created_at->format('d M Y, h:i a');
            })
            ->setRowId('id');
    }
    ...
    {
        $canExport = Auth::user()->hasPermissionTo('export designation management');
        $export = [];
        if ($canExport === true) {
            $export = [
                [
        ...
        return $this->builder()
            ...
            ->drawCallback("function() { " . file_get_contents(public_path('assets/js/custom/configurations/designations/listCallbacks.js')) . " }");
    }
    ...
    {
        return 'WDesignations_' . date('YmdHis');
    }
}
